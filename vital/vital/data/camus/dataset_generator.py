...
from vital.utils.image.io import load_mhd,load_nii
...
class CrossValidationDatasetGenerator:
    ...
    def __call__(
        ...
        with h5py.File(output, "w") as dataset:
            ...
            # my changes
            for patient_id in tqdm(patient_ids, unit="patient", desc="Saving patient images"):
                self._write_patient_data(output, patient_id)
    ...
    def _write_patient_data(self, output_dir: Path, patient_id: str) -> None:
        available_views = [
            view
        ...
        for view in available_views:
            ...
            if self.flags[CamusTags.registered]:
                _, data_y_proc, data_x_proc = self.registering_transformer.register_batch(data_y, data_x)
            else:
            ...
            for i, (img, mask) in enumerate(zip(data_x_proc, data_y_proc)):
                view_dir = output_dir / patient_id / view
                (view_dir / "images").mkdir(parents=True, exist_ok=True)
                (view_dir / "masks").mkdir(parents=True, exist_ok=True)

                img_norm = (255 * (img - img.min()) / (img.max() - img.min() + 1e-8)).astype(np.uint8)
                mask_bin = (mask > 0).astype(np.uint8) * 255

                Image.fromarray(img_norm).save(view_dir / "images" / f"{i:03}.png")
                Image.fromarray(mask_bin).save(view_dir / "masks" / f"{i:03}.png")
    ...
    def _get_sequence_data(self, patient_id: str, view: str) -> Tuple[List[np.ndarray], List[np.ndarray], List[Real]]:
        ...
        sequence, info = load_nii(patient_folder / sequence_fn_template.format(""))
        sequence_gt, _ = load_nii(patient_folder / sequence_fn_template.format("_gt"))
